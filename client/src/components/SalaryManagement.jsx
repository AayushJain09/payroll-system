...
const SalaryManagement = () => {
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      axios
        .post("http://localhost:5000/salaries/search", { month, year })
        .then((res) => {
  ...
  const calculateTotalSalary = (employee) => {
    ...
    const days = payDays[employee._id] || 0;
    const [year, month] = selectedDate.split("-");
    const daysInMonth = getDaysInMonth(month, year);
  ...
  return (
    <section>
      <h1 className="text-5xl text-white mb-4">Generate Payroll</h1>
      <div className="mb-4">
          ...
          <table className="min-w-full bg-white text-black">
            <thead>
              <tr>
                ...
                <th className="py-2">Basic Pay</th>
                <th className="py-2">HRA</th>
                <th className="py-2">DA</th>
                <th className="py-2">Pay Days</th>
            ...
            <tbody>
              {filteredEmployees.map((employee) => (
                <tr key={employee._id}>
                  ...
                  <td className="border px-4 py-2">{employee.salary?.hra || 0}</td>
                  <td className="border px-4 py-2">{employee.salary?.da || 0}</td>
                  <td className="border px-4 py-2">
                    <input
                      type="number"
                      min="0"
                      max="31"
                      className="w-full py-2 px-3 border rounded text-black bg-slate-300"
                      value={payDays[employee._id] || ""}
                      onChange={(e) => handlePayDaysChange(e, employee._id)}
                    />
                  </td>
                  <td className="border px-4 py-2">
